...
export async function GET(request: Request, context: any) {
  try {
    let { year, conferenceId, platformId } = context.params
    ...
    const sql = `
      ...
      WHERE Platforms.id LIKE '${platformId}' AND Conferences.release_date LIKE '${year}%' AND Conferences.id LIKE '${conferenceId}%'
